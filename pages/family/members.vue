    ...
    <view class="header">
      ...
      <view class="header-right">
        <view
          ...
          @click="showAlarmModal = true"
        >
          <image
            class="icon-md"
            src="/static/family/warning.svg"
            mode="aspectFit"
          ></image>
        </view>
      </view>
    ...
    <scroll-view scroll-y class="main-content">
        ...
        <view class="family-card">
          <view class="card-info">
            <view class="cover-image-wrapper">
              <image
                class="cover-image"
                src="/static/family/family_cover.png"
                mode="aspectFill"
              >
              </image>
            </view>
          ...
          <view class="care-container">
            ...
            <text class="care-text"
              >家是避风的港湾，记得有空关心家人，按时吃饭，照顾好自己~</text
            >
          </view>
        </view>
      </view>
      ...
      <view class="section-padding action-grid" v-if="userRole == 'leader'">
        <view
          ...
          @click="
            navigateTo(`/pages/family/invite?groupId=${groupInfo.groupId}`)
          "
        >
          <image
            class="icon-sm mr-2"
            src="/static/family/person_add.svg"
            mode="aspectFit"
          ></image>
        ...
        <view
          class="action-btn"
          hover-class="action-btn-hover"
          @click="openApplyCenter"
        >
          <image
            class="icon-sm mr-2"
            src="/static/family/verified_user.svg"
            mode="aspectFit"
          ></image>
      ...
      <view class="section-padding members-section">
        <view class="section-header">
          ...
          <view class="badge"
            ><text class="badge-text">共 {{ memberList.length }} 人</text></view
          >
        </view>
        ...
        <view class="members-list">
          <view
            ...
            @click="goToHealthData(member)"
          >
            <view class="member-info">
              <view class="avatar-wrapper">
                <image
                  class="avatar"
                  :class="{ 'avatar-leader': member.role == 'leader' }"
                  :src="this.getAvatar(member.userId)"
                  mode="aspectFill"
                ></image>
                <view v-if="member.role == 'leader'" class="leader-badge"
                  >★</view
                >
              </view>
              <view class="name-role">
                ...
                <text
                  class="role"
                  :class="{ 'role-leader': member.role == 'leader' }"
                  >{{ member.role }}</text
                >
              </view>
            </view>
            <image
              ...
              src="/static/Home/right-arrow.svg"
              mode="aspectFit"
            >
            </image>
          </view>
      ...
      <button
        v-if="userRole == 'normal'"
        class="btn-manage"
        hover-class="btn-manage-hover"
        @click="handleQuit"
      >
        退出家庭组
      </button>
      <view class="section-padding footer-note">
        <view class="note-box">
          <text class="note-text"
            >如需解散家庭组或转让组长角色，请访问高级设置。成员可以随时退出小组，但将失去对共享主资源的访问权限。</text
          >
        </view>
      </view>
        ...
        <scroll-view scroll-y class="apply-list-scroll">
          ...
          >
            <view class="apply-top">
              <image
                class="apply-avatar"
                :src="getAvatar(apply.userName || apply.userNickname)"
                mode="aspectFill"
              ></image>
              ...
              <view class="apply-info">
                <view class="name-time">
                  <text class="name">{{
                    apply.userName || apply.userNickname
                  }}</text>
                  <text class="time">{{
                    apply.createTime.substring(0, 10)
                  }}</text>
                </view>
                <text class="remark"
                  >“{{ apply.remark || "申请加入您的家庭组" }}”</text
                >
              </view>
            </view>
            ...
            <view class="apply-actions" v-if="apply.status === 'pending'">
              <button
                class="btn-reject"
                hover-class="btn-reject-hover"
                @click="handleApply(apply, 'reject')"
              >
                拒绝
              </button>
              <button
                class="btn-agree"
                hover-class="btn-agree-hover"
                @click="handleApply(apply, 'accept')"
              >
                同意
              </button>
            </view>
            ...
            <view class="alarm-top">
              ...
              <view class="alarm-info">
                <view class="name-time">
                  ...
                  <text class="time">{{
                    alarm.alarmTime.substring(0, 16)
                  }}</text>
                </view>
                <text class="remark">
                  漏服药品：<text class="medicine-name">{{
                    alarm.medicineName
                  }}</text>
                </text>
              </view>
  ...
  methods: {
    ...
    async handleApply(apply, action) {
      ...
      try {
        ...
        const res = await familyApi.approveApply(
    ...
    async fetchData() {
      ...
      try {
        ...
        if (res.members && Array.isArray(res.members)) {
          const matchedMember = res.members.find(
    ...
    goToHealthData(member) {
      ...
      const targetData = this.healthData.find(
        (item) => item.userId === member.userId
      );

      // 2. 将找到的对象转为 JSON 字符串并进行 URI 编码
      // 如果没找到对应的数据，默认传一个空的编码字符串，防止报错
      const healthDataString = targetData
...
.care-container {
  margin-top: 20rpx;
  padding: 16rpx 20rpx;
  background-color: rgba(255, 255, 255, 0.5); /* 半透明背景增加质感 */
  border-radius: 12rpx;
  display: flex;
  align-items: center;
}
