<template>
  <scroll-view
    ...
    :style="{ '--base-font': globalFontSize + 'px' }"
  >
    ...
    <view class="header">
      <view class="user-section">
        ...
        <view class="notification-btn" @tap="toNotification">
          <view
            class="icon"
            style="background-image: url('static/Home/bell.svg')"
          ></view>
    ...
    <view class="section-box">
      <view class="section-title">
        ...
        <view class="more-link" @tap="toMedicationList">
          <text
            style="font-size: calc(var(--base-font) + 2rpx)"
            @click="uni.navigateTo({ url: '/pages/reminder/Reminder' })"
            >任务中心</text
          >
      ...
      <view v-else class="task-area">
        ...
        <view class="med-card" :class="getStatusClass(taskList[0].status)">
          ...
          <view class="med-action">
            ...
            <button
              v-else-if="taskList[0].status === 0"
              class="take-btn"
              @tap="handleTaskClick(taskList[0], 0)"
            >
              服用
            </button>
            <text
              v-else-if="taskList[0].status === 2"
              class="status-missed-text"
              >！漏服</text
            >
          </view>
        </view>
...
export default {
  ...
  methods: {
    ...
    toMedicationList() {
      uni.navigateTo({ url: "/pages/medication/list" });
    },
    toHealthDetail() {
    ...
    _startBlobAnimation() {
      ...
      const borderFrames = [
        {
          outer: "60% 40% 30% 70% / 60% 30% 70% 40%",
          inner: "50% 60% 40% 50% / 40% 50% 60% 50%",
        },
        {
          outer: "30% 60% 70% 40% / 50% 60% 30% 60%",
          inner: "60% 40% 50% 60% / 60% 40% 50% 40%",
        },
        {
          outer: "50% 50% 40% 60% / 30% 70% 50% 50%",
          inner: "40% 60% 60% 40% / 50% 40% 60% 60%",
        },
        {
          outer: "70% 30% 50% 50% / 60% 40% 60% 40%",
          inner: "55% 45% 45% 55% / 45% 55% 45% 55%",
        },
      ];
      let i = 0;
      const next = () => {
