...
│                                                    │
│  ┌──────────────────┐  ┌──────────────────┐       │
│  │ medicationTaskPool│  │  remindTaskPool   │       │
│  │ ConcurrentHashMap │  │ ConcurrentHashMap │       │
│  │ <Long, Future>    │  │ <Long, Future>    │       │
│  └──────────────────┘  └──────────────────┘       │
│  ┌──────────────────┐                              │
│  │  inviteTaskPool   │   全部在 JVM 堆内存中       │
│  │ ConcurrentHashMap │   → 重启即丢失              │
│  │ <Long, Future>    │                              │
│  └──────────────────┘                              │
│                                                    │
│  @PostConstruct init()                             │
│  → 扫描 DB 今天的任务，创建 ScheduledFuture        │
│  → 重启后定时器全部重建（精度丢失）                │
│                                                    │
...
```
┌─────────────────────────────────────────────────────────────────┐
│                     DynamicTaskScheduler (Publisher)              │
│  职责：扫描 DB → 发布 DelayTask 到 Redis ZSET                    │
│  - @PostConstruct init(): 启动时扫描今天任务并发布               │
│  - @Scheduled cron: 每日刷新                                     │
│  - addTaskSchedule() / cancelTaskSchedule()                      │
│  - addInviteExpireTask() / cancelInviteExpireTask()              │
└──────────────────────────┬──────────────────────────────────────┘
                           │ publish(DelayTask)
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│                   Redis ZSET: infra:delay:tasks                   │
│  Score = executeAt.toEpochMilli()   Member = DelayTask JSON      │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │ Score: 1715846700000  │ {taskType:"medication.remind", ...} │ │
│  │ Score: 1715847000000  │ {taskType:"medication.missed", ...} │ │
│  │ Score: 1716019200000  │ {taskType:"family.invite.expire",..}│ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                                                   │
│  Redis 持久化 (RDB/AOF) → 重启不丢失                             │
│  Lua 原子操作 (ZRANGEBYSCORE + ZREM) → 多实例安全                │
└──────────────────────────┬──────────────────────────────────────┘
                           │ poll every 1s (Lua atomic pop)
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│              RedisDelayTaskConsumer (Scheduler)                   │
│                                                                   │
│  pollReadyTasks()                                                │
│    ├── 检查取消标记: infra:delay:canceled Set                    │
│    ├── 匹配 DelayTaskHandler                                     │
│    ├── 执行成功 → 完成                                           │
│    └── 执行失败 → 指数退避重试 (2^retryCount 秒, max 3次)       │
│                    └── 重试耗尽 → RabbitMQ DLQ                   │
└──────────────────────────┬──────────────────────────────────────┘
          ...
          ▼                ▼                ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│ Medication   │ │ Medication   │ │ InviteExpire │
│ RemindHandler│ │ MissedHandler│ │ Handler      │
│              │ │              │ │              │
│ type:        │ │ type:        │ │ type:        │
│ medication.  │ │ medication.  │ │ family.      │
│ remind       │ │ missed       │ │ invite.expire│
│              │ │              │ │              │
│ 发布WS提醒   │ │ 标记漏服+WS  │ │ 标记过期     │
│ 推送事件     │ │ 推送告警事件 │ │ 状态更新     │
└──────────────┘ └──────────────┘ └──────────────┘
          ...
          ▼                ▼                ▼
┌─────────────────────────────────────────────────────────────────┐
│               RabbitMQ DLX: medical.dlx.topic                    │
│  重试耗尽的任务进入死信队列，可手动重发                          │
└─────────────────────────────────────────────────────────────────┘
